...
var (
	...
	argMetricHost = pflag.String("metric-host", "", "The address of the metrics Apiserver "+
		"to connect to in the format of protocol://address:port, e.g., "+
		"http://localhost:8082. If not specified, the assumption is that the binary runs inside a "+
		"Kubernetes cluster and service proxy will be used.")
	argMetricProvider     = pflag.String("metric-provider", "heapster", "Select provider type for metrics. Defaults to 'heapster'")
...
func main() {
	...
	metricHost := args.Holder.GetMetricHost()
	metricProvider := args.Holder.GetMetricProvider()

	if heapsterHost := args.Holder.GetHeapsterHost(); 0 != len(heapsterHost) {
		metricHost = heapsterHost
		metricProvider = "heapster"
	}

	metricIntegrationID := integrationapi.HeapsterIntegrationID
	switch metricProvider {
	case "sidecar":
		metricIntegrationID = integrationapi.SidecarIntegrationID
	case "heapster":
		metricIntegrationID = integrationapi.HeapsterIntegrationID
	default:
		log.Printf("Unrecognizable metric provider, using default integration id: %s", metricIntegrationID)
	}

	integrationManager.Metric().ConfigureMetric(metricProvider, metricHost).
		EnableWithRetry(metricIntegrationID, time.Duration(args.Holder.GetMetricClientCheckPeriod()))
...
func initArgHolder() {
	...
	builder.SetMetricsHost(*argMetricHost)
	builder.SetMetricProvider(*argMetricProvider)
